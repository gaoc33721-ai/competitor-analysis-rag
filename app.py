...
with st.sidebar:
    st.markdown("**测试用例参考（聚焦素材图与营销策略）：**\n"
                "- 给我推荐一些优秀的冰箱详情页主图素材，分析它们在视觉设计上有什么亮点？\n"
                "- 对比一下库里 TCL 和三星的电视产品图，它们在传达【高端感】的视觉手法上有什么不同？\n"
                "- 帮我找几个通过【场景代入】来展现产品特点的洗衣机素材，说明为什么这种排版好。\n"
                "- 我想设计一款针对租房年轻人的小家电素材，库里有什么能让我参考的构图和文案？\n"
                "- 过滤掉LG品牌，帮我找一张在【参数可视化】上做得最好的竞品主图。")
        ...
        with st.spinner("🔍 正在检索素材库并生成策略分析..."):
            try:
                ...
                prompt = PromptTemplate(
                    ...
                    8. **严格区分【视觉素材】与【文本文字】**：当用户提问“素材/图片/主图”时，你必须**100%聚焦于视觉层面的拆解**（例如：构图排版、场景化代入感、参数可视化呈现、色彩搭配、规模对比等）。**绝对禁止**把大段篇幅用来分析“这句文案写得多好”、“文字描述了什么卖点”。除非用户明确提问“文案”，否则请把所有焦点放在“这张图是怎么做视觉营销的”！
                    9. **强制输出图片编号**：你必须、必须在回答的最后一行，按照规定格式输出相关素材的编号，以便前端系统能正确渲染图片！如果因为你的疏忽导致系统没抓到编号，前端将只显示干巴巴的文字，这是严重的事故！
                    10. **品牌绝对一致性**：最后输出的 [相关素材编号] 必须且只能属于用户提问的品牌！如果你在正文中分析了 Ninja，绝对不允许在最后输出的三星素材编号！如果该品牌没有合适的素材图，宁可输出“无”，也绝不能拿其他品牌的素材来充数！
                    11. **拒绝无意义推荐**：如果你发现候选素材虽然是某个品牌，但它并不是你真正想推荐的那个产品（例如，你分析的是 Ninja 空气炸锅，但候选素材里只有一包 True Fresh 的洗洁精），你必须直接输出 `[相关素材编号]: 无`，绝对不允许为了配图而随便指派一个毫不相干的图片编号！
                ...
                # 移除回答中给程序看的特殊标记
                # 修改正则：把 [相关素材编号]: 4 以及后面的所有内容（包括换行）全部抹掉，只展示纯净的文字分析
                ...
                # 必须传递 unique_filtered_images，因为后续渲染用的是这个去重后的列表
                st.session_state.messages.append({
                    "role": "assistant",
                    "content": display_answer, # 这里存入 display_answer 而不是原始 answer，防止历史记录中露出 [相关素材编号]
